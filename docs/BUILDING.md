...
SoundLine client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/SoundLine/SoundLine
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/SoundLine/SoundLine/discussions/2840)
...
Also make sure you have built the `@SoundLine/core` workspace before running E2E tests.
...
```shell
# there are `SoundLine-local`, `SoundLine-migration`, `SoundLine-local`, `SoundLine-prototype` e2e tests,
#   which are run under different situations.
cd tests/SoundLine-local
yarn e2e
```
